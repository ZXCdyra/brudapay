...
const (
	...
	// RoleMerchant — внешний клиент со своим кабинетом. Конкретный вид кабинета
	// определяется полем Casino.MerchantType (merchant — даёт трафик,
	// trader — принимает трафик и держит реквизиты).
	RoleMerchant Role = "MERCHANT"
)

// MerchantType задаёт тип внешнего аккаунта.
type MerchantType string

const (
	// MerchantTypeMerchant — мерчант: даёт трафик (создаёт депозиты),
	// собственных реквизитов не имеет.
	MerchantTypeMerchant MerchantType = "merchant"
	// MerchantTypeTrader — трейдер: принимает трафик, заводит реквизиты
	// и вносит страховой депозит.
	MerchantTypeTrader MerchantType = "trader"
)


// IsStaff возвращает true для внутренних ролей площадки (не мерчант).
func (r Role) IsStaff() bool {
	switch r {
	case RoleOwner, RoleSuperAdmin, RoleAdmin, RoleSupport, RoleAnalyst:
		return true
	}
	return false
}
...
type User struct {
	...
	// CasinoID связывает пользователя-мерчанта с его записью в casinos.
	// Для внутреннего персонала (owner/admin/...) равно nil.
...
type Casino struct {
	...
	// MerchantType определяет вид внешнего аккаунта: "merchant" (даёт трафик)
	// или "trader" (принимает трафик и держит реквизиты).
