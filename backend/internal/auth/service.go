...
import (
	...
	"github.com/brudapay/brudapay/pkg/crypto"
...
func (s *Service) Login(ctx context.Context, req LoginRequest, ip string) (*TokenResponse, error) {
	...
	err := s.db.Pool.QueryRow(ctx, `
		SELECT id, email, password_hash, role, status, casino_id, created_at, updated_at
		FROM users WHERE email = $1
	`, req.Email).Scan(
		&user.ID, &user.Email, &user.PasswordHash, &user.Role,
		&user.Status, &user.CasinoID, &user.CreatedAt, &user.UpdatedAt,
	)
