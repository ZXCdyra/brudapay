...
class ApiClient {
  ...
  // Merchant cabinet (личный кабинет мерчанта/трейдера)
  getMerchantDashboard() {
    return this.request<import("@/types").MerchantDashboard>("/merchant/dashboard");
  }

  getMerchantProfile() {
    return this.request<import("@/types").MerchantProfile>("/merchant/profile");
  }

  getMerchantTransactions(params?: Record<string, string>) {
    const q = params ? "?" + new URLSearchParams(params).toString() : "";
    return this.request<import("@/types").Transaction[]>(`/merchant/transactions${q}`);
  }

  getMerchantBalance() {
    return this.request<{ balance: number; frozen_balance: number; currency: string }>(
      "/merchant/balance"
    );
  }

  getMerchantRequisites() {
    return this.request<import("@/types").Requisite[]>("/merchant/requisites");
  }

  createMerchantRequisite(data: {
    bank_name: string;
    holder_name: string;
  ...
  deleteMerchantRequisite(id: string) {
    return this.request(`/merchant/requisites/${id}`, { method: "DELETE" });
  }

  topUpInsurance(amount: number) {
    return this.request<import("@/types").MerchantProfile>("/merchant/insurance/topup", {
      method: "POST",
